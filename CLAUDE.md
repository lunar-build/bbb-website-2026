...
- **px → rem accessibility (WCAG 1.4.4/1.4.10):** theme SCSS was swept once (PR #14) to convert font-size, line-height, spacing, and icon/logo/button dimensions from px to rem so layout scales with browser zoom/font-size. That sweep is a point-in-time fix, not enforced — new SCSS in later PRs can reintroduce raw px. Until a lint rule exists to catch this automatically (e.g. stylelint `declaration-property-unit-disallowed-list` blocking `px` on those properties), any block/component work should follow the rem-first rule already documented in the `build-acf-block` skill ("Sizing: use rem, not px"), and periodic `grep -rn '[0-9]px' resources/styles --include='*.scss'` sweeps should be run to catch drift — exclude borders/outlines (1-2px), box-shadow, `999px` pill radii, and `.u-sr-only`'s clip pattern, which are intentionally px.
