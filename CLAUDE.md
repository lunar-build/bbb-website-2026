...
- Blocks live in `app/Blocks/*.php` (e.g. `app/Blocks/TextHero.php`), each extending `Log1x\AcfComposer\Block`. Public properties configure block metadata (name, category, icon, supports, alignment, styles, `example` preview data, `template` for default inner blocks). `fields()` builds the ACF field group via `Builder::make(...)`. `with()` returns data passed into the block's Blade view. Accessor methods per ACF field (e.g. `CtaStrip::link()`) wrap `get_field()` with fallbacks to `$this->example` — only needed for fields actually defined in `fields()`; don't leave stale accessors around for fields that have been removed (a block using `InnerBlocks` for its copy, like `TextHero`, has no accessor for that copy at all).
- Each block's Blade view lives at `resources/views/blocks/<kebab-block-name>.blade.php`.
- Standalone field groups (not tied to a block) go in `app/Fields/`. Global options pages (settings not tied to a specific block or post) go in `app/Options/*.php`, each extending `Log1x\AcfComposer\Options` — see `app/Options/ThemeOptions.php` for the first (and so far only) example, read via `get_field('field_name', 'option')`.
